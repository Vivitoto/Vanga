...
class CoilAwareDecoder(private val imageDecoder: VangaImageDecoder) {

    suspend fun decodeFromFile(path: Path, options: Options): VangaImage {
    ...
    suspend fun decodeBytes(bytes: ByteArray, options: Options): VangaImage {
        if (options.size.isOriginal) {
...
expect suspend fun VangaImage.toCoilImage(): Image