...
import io.github.vivitoto.vanga.ui.readlist.BookReadListsState
...
class OneshotViewModel(
    ...
    suspend fun initialize() {
        ...
        book.filterNotNull().combine(libraries) { book, libraries ->
            ...
            if (newLibrary == null) {
                mutableState.value =
                    Error(IllegalStateException("找不到单本《${book.metadata.title}》所属书库"))
            }
    ...
    private fun getLibraryOrThrow(book: VangaBook): KomgaLibrary {
        val library = this.libraries.value.firstOrNull { it.id == book.libraryId }
        if (library == null) {
            throw IllegalStateException("找不到单本《${book.metadata.title}》所属书库")
        }
        return library
    }
