...
import io.github.vivitoto.vanga.ui.vanga_ui.generated.resources.Res
...
import io.github.vivitoto.vanga.settings.model.TtsuUserFont
...
class TtsuReaderState(
    ...
    private val webview = MutableStateFlow<VangaWebview?>(null)
    ...
    override fun onWebviewCreated(webview: VangaWebview) {
    ...
    @OptIn(ExperimentalResourceApi::class)
    private suspend fun loadEpub(webview: VangaWebview) {
        ...
        webview.registerRequestInterceptor { request ->
            runCatching {
                ...
                when {
                    urlString == "http://vanga/ttsu.html" -> {
                        val bytes = Res.readBytes("files/ttsu.html")
                        ResourceLoadResult(data = bytes, contentType = "text/html")
                    }

                    urlString == "http://vanga/favicon.ico" -> null

                    urlString.startsWith("http://vanga/userfonts") -> {
                    ...
                    urlString.startsWith("http://vanga") -> error("无效的请求 URI：$urlString")
                    else -> proxyResourceRequest(bookApi, urlString, serverUrl)
                }
            }.onFailure { logger.catching(it) }.getOrNull()
        }

        webview.navigate("http://vanga/ttsu.html")
        webview.start()
    }
        ...
        for (resource in manifest.resources) {
            ...
            while (iter.hasNext()) {
                ...
                val srcAttr = when (image.tag().normalName()) {
                    ...
                    else -> error("意外的图片标签：${image.tag().name()}")
                }
