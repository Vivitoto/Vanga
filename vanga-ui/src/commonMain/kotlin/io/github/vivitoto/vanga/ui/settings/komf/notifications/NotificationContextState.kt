...
class NotificationContextState {
    ...
    var seriesAuthors by mutableStateOf(
        listOf(
            AuthorContext("作者1", "编剧"),
            AuthorContext("作者2", "画师")
        )
    )
    var seriesReleaseYer by mutableStateOf<Int?>(1970)
    var seriesLinks by mutableStateOf(listOf(WebLinkContext("示例链接", "http://example.com")))
    ...
    class BookContextState {
        ...
        var name by mutableStateOf("测试单本")
        var number by mutableStateOf(1)
        var title by mutableStateOf("单本元数据标题")
        var summary by mutableStateOf("单本简介")
    ...
    class AlternativeTitleContext(label: String = "En", title: String = "别名") {
        var label by mutableStateOf(label)
        var title by mutableStateOf(title)
    }

    class AuthorContext(name: String = "作者", role: String = "作者角色") {
        var name by mutableStateOf(name)
        var role by mutableStateOf(role)
    }

    class WebLinkContext(label: String = "示例链接", url: String = "http://example.com") {
        var label by mutableStateOf(label)
