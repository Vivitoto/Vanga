...
HWND get_awt_hwnd(JNIEnv *env, jobject awt_window) {
    ...
    if (dsi == NULL) {
        ...
        if (wdsi != NULL) {
            ...
            vanga_throw_jvm_exception(env, "Can't get w32 platform info");
        }
...
webview_t vanga_webview_create(JNIEnv *env, jobject awt_window) {
...
void vanga_webview_destroy_callback(webview_t webview, void *data) {
    g_mutex_lock(&mutex);
...
void vanga_webview_destroy(vanga_webview_t data) {
    ...
    if (webview_data->interceptor != NULL) { vanga_interceptor_destroy(webview_data->interceptor); }
    g_hash_table_destroy(webview_data->bind_callbacks);

    SetParent(webview_data->webview_window->native_window, NULL);
    webview_dispatch(webview_data->webview, vanga_webview_destroy_callback, webview_data);
}
...
void vanga_webview_bind_cb(webview_t webview, void *data) {
...
void vanga_webview_bind(vanga_webview_t webview, bind_callback_t *callback) {
    ...
    webview_dispatch(webview_data->webview, vanga_webview_bind_cb, callback);
}
...
HRESULT RequestHandlerInvoke(
    ...
    resources.result = vanga_interceptor_run(static_interceptor, uri_utf8);
...
void vanga_register_interceptor_cb(webview_t webview, void *) {
...
void vanga_register_request_interceptor(vanga_webview_t webview, request_interceptor *interceptor) {
    ...
    webview_dispatch(webview_data->webview, vanga_register_interceptor_cb, NULL);
