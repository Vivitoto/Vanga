...
JNIEXPORT jobject JNICALL Java_snd_vanga_image_AndroidBitmap_createHardwareBuffer(
  ...
  if (allocation_error) {
    vanga_throw_jvm_vips_exception_message(env, "Could not allocate bitmap hardware buffer");
    g_object_unref(processed_input);
    return nullptr;
  }
  ...
  if (lock_error) {
    vanga_throw_jvm_vips_exception_message(env, "Could not acquire created hardware hardware_buffer");
  ...
  if (unlock_error) {
    vanga_throw_jvm_vips_exception_message(env, "Failed to unlock hardware buffer");
    AHardwareBuffer_release(hardware_buffer);
    return nullptr;
  }
...
JNIEXPORT jobject JNICALL Java_snd_vanga_image_AndroidBitmap_createSoftwareBitmap(
  ...
  if (lock_error) {
    vanga_throw_jvm_vips_exception_message(env, "Failed to lock Bitmap");
    g_object_unref(processed_image);
    return nullptr;
  }
  ...
  if (unlock_error) {
    vanga_throw_jvm_vips_exception_message(env, "Failed to unlock Bitmap");
    return nullptr;
  }
