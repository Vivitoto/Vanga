...
jobject get_jvm_enum_type(JNIEnv *env, VipsImage *image) {
  ...
  case VIPS_INTERPRETATION_HISTOGRAM:
    ...
    break;
  default:
    vanga_throw_jvm_vips_exception_message(env, "unsupported vips interpretation");
    return nullptr;
  }
...
VipsImage *vanga_from_jvm_handle(JNIEnv *env, jobject jvm_image) {
  ...
  if (image == nullptr) {
    vanga_throw_jvm_vips_exception_message(env, "image was already closed\n");
    return nullptr;
  }
...
jobject
vanga_to_jvm_handle(JNIEnv *env, VipsImage *image, const unsigned char *external_source_buffer) {
