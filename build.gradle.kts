...
val resourcesDir = "$projectDir/vanga-infra/jni/src/jvmMain/resources/"
val androidJniLibsDir = "$projectDir/vanga-infra/jni/src/androidMain/jniLibs"
val composeDistroResourcesDir = "$projectDir/vanga-app/desktopUnpackedResources"
val composeCommonResources = "$projectDir/vanga-ui/src/commonMain/composeResources/files"

val epubReader = "$rootDir/vanga-epub-reader"
...
val androidLibs = linuxCommonLibs + setOf(
    "libvanga_android_bitmap.so",
    "libiconv.so",
    "libomp.so",
)
val desktopLinuxLibs = linuxCommonLibs + setOf(
    "libvanga_webview.so",
    "libvanga_webkit_extension.so",
)
...
tasks.register<Sync>("android-aarch64_copyJniLibs") {
    group = "jni"
    dependsOn(":vanga-infra:database:sqlite:android-arm64-ExtractSqliteLib")
...
tasks.register<Sync>("android-armv7a_copyJniLibs") {
    group = "jni"
    dependsOn(":vanga-infra:database:sqlite:android-armv7a-ExtractSqliteLib")
...
tasks.register<Sync>("android-x86_64_copyJniLibs") {
    group = "jni"
    dependsOn(":vanga-infra:database:sqlite:android-x86_64-ExtractSqliteLib")
...
tasks.register<Sync>("android-x86_copyJniLibs") {
    group = "jni"
    dependsOn(":vanga-infra:database:sqlite:android-x86-ExtractSqliteLib")
...
tasks.register<Exec>("cmakeSystemDepsConfigure") {
    ...
    inputs.file("$projectDir/vanga-infra/image-decoder/vips/native/CMakeLists.txt")
    inputs.file("$projectDir/vanga-infra/webview/native/CMakeLists.txt")
    commandLine(
        "cmake",
        "-B", "cmake-build",
        "-G", "Ninja",
        "-DCMAKE_BUILD_TYPE=Release",
        "-DVANGA_SUPERBUILD=OFF"
    )
}
...
tasks.register<Exec>("cmakeSystemDepsBuild") {
    ...
    outputs.dir("$projectDir/cmake-build/vanga-infra/image-decoder/native")
    outputs.dir("$projectDir/cmake-build/vanga-infra/webview/native")
    commandLine(
        "cmake",
        "--build",
        "cmake-build",
...
tasks.register<Sync>("cmakeSystemDepsCopyJniLibs") {
    ...
    inputs.dir("$projectDir/cmake-build/vanga-infra/webview/native")
    inputs.dir("$projectDir/cmake-build/vanga-infra/image-decoder/vips/native")
    outputs.dir(resourcesDir)

    from(
        "$projectDir/cmake-build/vanga-infra/image-decoder/vips/native",
        "$projectDir/cmake-build/vanga-infra/webview/native"
    )
    into(resourcesDir)
...
tasks.register("vangaBuildNonJvmDependencies") {
    group = "build"
