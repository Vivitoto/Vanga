...
This is Vanga's durable workaround for this workstation, where normal
`git push`/`git fetch` over HTTPS can time out even though `gh api` works.

Default behavior:
- Pushes local commits to Vivitoto/Vanga main via GitHub's Git Data API.
...
DEFAULT_REPO = os.environ.get("VANGA_REPO", "Vivitoto/Vanga")
DEFAULT_BRANCH = os.environ.get("VANGA_BRANCH", "main")
